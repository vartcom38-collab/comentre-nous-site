      ...
      <main className="product-page-rich">
        <section className="product-hero-rich">
          ...
          <div className="product-info-rich">
            ...
            {product.wooProductId ? <button type="button" className="dark-btn purchase-btn woo-add-btn" data-woo-add={product.wooProductId} data-product-title={product.title}>Ajouter au panier →</button> : purchaseUrl ? <a className={`dark-btn purchase-btn ${product.purchaseChannel || 'external'}`} href={purchaseUrl} target="_blank" rel="noopener noreferrer">{purchaseLabel} →</a> : <span className="dark-btn disabled-buy">Bientôt disponible</span>}
            {product.wooProductId ? <p className="purchase-note">Paiement sécurisé et commande gérés directement depuis la boutique.</p> : <>{product.purchaseChannel === 'amazon' && <p className="purchase-note">Achat et livraison gérés sur Amazon.</p>}{product.purchaseChannel === 'stripe' && <p className="purchase-note">Paiement sécurisé via Stripe.</p>}</>}
          </div>
        </section>
