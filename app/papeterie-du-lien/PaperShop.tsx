...
function inferredTabs(product:PaperProduct){
  if(product.paperTabs?.length){
    return product.paperTabs.map(id=>id==='journaux'?'bujo':id);
  }
  const hay=`${product.type} ${product.title} ${product.tagline} ${product.format}`.toLowerCase();
  const tabs:string[]=[];
  if(/bujo|bullet|bullet journal|carnet|journal|planner|agenda/.test(hay))tabs.push('bujo');
  ...
  return tabs.length?Array.from(new Set(tabs)):['organisation'];
}
