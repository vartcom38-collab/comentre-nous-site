...
export default function PapeterieNotificationsPage(){
  ...
  useEffect(()=>{
    ...
    setAdminToken(saved);
    if(saved) load(saved);
    else setStatus('Reconnecte-toi depuis l’admin général pour modifier cette adresse.');
  },[]);
  ...
  async function load(token=adminToken){
    ...
    try{
      const r=await fetch('/api/comentre/order-notifications.php',{headers:{'X-Admin-Github-Token':token},cache:'no-store'});
      const d=await r.json();
      if(!r.ok)throw new Error(d.error||'Connexion impossible');
      setEmail(d.email||'');setReady(true);setStatus('');
    }catch(e){
      setReady(false);
      const message=(e as Error).message;
      setStatus(message==='admin_login_required'||message==='admin_forbidden'?'Reconnecte-toi depuis l’admin général pour continuer.':`Erreur : ${message}`);
  ...
  async function save(){
    ...
    try{
      const r=await fetch('/api/comentre/order-notifications.php',{method:'POST',headers:{'Content-Type':'application/json','X-Admin-Github-Token':adminToken},body:JSON.stringify({action:'save',email:email.trim()})});
      const d=await r.json();
  ...
  async function test(){
    ...
    try{
      const r=await fetch('/api/comentre/order-notifications.php',{method:'POST',headers:{'Content-Type':'application/json','X-Admin-Github-Token':adminToken},body:JSON.stringify({action:'test',email:email.trim()})});
      const d=await r.json();
  ...
  return <main className="notify-admin"><style>{css}</style>
    ...
    {!ready?<section className="card connect"><h2>Accès protégé</h2><p>Cette page utilise automatiquement ta connexion à l’admin du site. Si tu viens d’ouvrir cette page directement, reconnecte-toi simplement depuis l’admin général puis reviens ici.</p><div className="actions"><Link className="primary-link" href="/admin/">Ouvrir l’admin</Link><button onClick={()=>load()} disabled={busy||!adminToken}>{busy?'Vérification…':'Réessayer'}</button></div></section>:
