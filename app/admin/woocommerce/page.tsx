...
type WooProduct={id:number|null;name:string;slug:string;status:string;price:string;regular_price:string;sale_price:string;stock_status:string;stock_quantity:number|null;featured:boolean;permalink:string;images:Array<{id:number|null;src:string;alt:string}>;categories:Array<{id:number|null;name:string;slug:string}>;meta_data:Array<{id?:number;key?:string;value?:unknown}>};
...
export default function WooCommerceAdminPage(){
  ...
  async function load(){setLoading(true);setError('');try{const response=await fetch(`/woo-bridge.php?action=products&ts=${Date.now()}`,{cache:'no-store'});const data=await response.json() as WooResponse;if(!response.ok||!data.ok)throw new Error(data.error||'Connexion WooCommerce impossible');setProducts(data.products||[]);if(!wooId&&data.products?.[0]?.id)setWooId(String(data.products[0].id));}catch(err){setError((err as Error).message)}finally{setLoading(false)}}
  ...
  async function saveLink(){
    ...
    setLinking(true);setLinkStatus('Enregistrement de la liaison…');
    try{
      let saved:WooLinks|null=null;
      for(let attempt=0;attempt<3&&!saved;attempt+=1){
        const response=await fetch(`https://api.github.com/repos/${OWNER}/${REPO}/contents/${LINKS_PATH}?ref=${BRANCH}&ts=${Date.now()}`,{cache:'no-store',headers:{Accept:'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28',Authorization:`Bearer ${token}`}});
        if(!response.ok)throw new Error(`${response.status} ${response.statusText}`);
        const current=await response.json();
        const latest=JSON.parse(decodeBase64(current.content)) as WooLinks;
        const next={...latest,[localId]:Number(wooId)};
        const put=await fetch(`https://api.github.com/repos/${OWNER}/${REPO}/contents/${LINKS_PATH}`,{method:'PUT',headers:{Accept:'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28',Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({message:'Admin: link product to WooCommerce',content:encodeBase64(`${JSON.stringify(next,null,2)}\n`),sha:current.sha,branch:BRANCH})});
        if(put.ok){saved=next;break}
        if(put.status!==409)throw new Error(`${put.status} ${put.statusText}`);
        await new Promise(resolve=>setTimeout(resolve,500));
      }
      if(!saved)throw new Error('Conflit GitHub persistant. Réessaie dans quelques secondes.');
      setLinks(saved);
      setLinkStatus('Liaison enregistrée ✓ Tu peux la modifier à tout moment en choisissant un autre produit WooCommerce.');
    }catch(err){setLinkStatus(`Erreur liaison : ${(err as Error).message}`)}finally{setLinking(false)}
  }
  ...
  async function syncCommercial(){
    ...
    try{const payload:{id:number;regular_price:string;featured:boolean;status:string;stock_status:string;stock_quantity?:number}={id:linkedWooId,regular_price:price,featured:Boolean(localProduct.featured),status:localProduct.published?'publish':'draft',stock_status:wooStock(localProduct.stockStatus)};if((localProduct.stockQuantity??0)>0)payload.stock_quantity=localProduct.stockQuantity;const response=await fetch('/woo-bridge.php?action=update-product',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify(payload)});const data=await response.json() as WooResponse;if(!response.ok||!data.ok)throw new Error(data.error||'Synchronisation WooCommerce impossible');setLinkStatus('Synchronisation réussie ✓ Prix, stock, statut et mise en avant sont passés dans WooCommerce.');await load();}catch(err){setLinkStatus(`Erreur synchronisation : ${(err as Error).message}`)}finally{setSyncing(false)}
  }
  ...
  return <main className="woo-admin"><style>{css}</style>
    ...
    <section className="woo-linker"><div className="linker-title"><div><p>Liaison produits</p><h2>Relier une fiche Com’ entre nous à WooCommerce</h2><span>Tu peux modifier une liaison existante simplement en choisissant un autre produit WooCommerce.</span></div><b>SÉCURISÉ</b></div><div className="link-grid"><label>Fiche sur le nouveau site<select value={localId} onChange={e=>setLocalId(e.target.value)}>{localProducts.map(p=><option key={p.id} value={p.id}>{p.title} · {p.price||'prix à définir'}</option>)}</select></label><label>Produit WooCommerce<select value={wooId} onChange={e=>setWooId(e.target.value)}>{products.map(p=><option key={p.id??p.slug} value={String(p.id)}>{p.name} · {p.price?`${p.price} €`:'sans prix'}</option>)}</select></label></div><div className="link-actions"><button onClick={saveLink} disabled={linking}>{linking?'Enregistrement…':linkedWooId?'Modifier la liaison':'Enregistrer la liaison'}</button>{linkedWooId&&<button className="sync" onClick={syncCommercial} disabled={syncing}>{syncing?'Synchronisation…':'Tester la synchro commerciale'}</button>}</div>{localProduct&&<div className="compare"><div><small>Notre fiche</small><b>{localProduct.title}</b><span>Prix {localProduct.price||'—'} · {localProduct.published?'Publié':'Brouillon'} · {localProduct.featured?'Coup de cœur':'Standard'}</span></div><strong>→</strong><div><small>WooCommerce {linkedWoo?'relié':'à relier'}</small><b>{linkedWoo?.name||selectedWoo?.name||'Choisir un produit'}</b><span>{linkedWoo?`Prix ${linkedWoo.price||'—'} € · ${linkedWoo.stock_status}`:'La liaison seule ne modifie rien.'}</span></div></div>}{linkStatus&&<div className="link-status">{linkStatus}</div>}</section>
